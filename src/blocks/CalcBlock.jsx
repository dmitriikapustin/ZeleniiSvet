...
function Counter({ from, to, text }) {
	...
	useEffect(() => {
		...
		onUpdate(value) {
			...
			node.textContent = newValue.toFixed(2);
		}
		});
	...
	return <div className='div-result font-5-bold'><p className='p-result font-5-bold' ref={nodeRef}/>  руб</div>
}
...
const CalcBlock = () => {
	...
	function calc(sum, term, rate) {
		...
		let calcResult = Math.round((S * n / (1 - Math.pow(1 + n, -p))).toFixed(2)/12);
